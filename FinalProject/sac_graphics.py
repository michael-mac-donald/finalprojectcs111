...
# Version 5.1.2 11/14/2022
#   * Added a getFill() and a getOutline() method to the base object class
#   * at the request of some students that want to use the colors for semantic
#   * values in their programs.
#   aalbina

# Version 5.1.1 8/28/2022
#   * Made it so the created window is now brought automatically to the top.
#   * Added a move_to function for elements, this avoids the need to using the
#       move delta for formatting.
#   * Moves on polygons change the locations of the center.
#   * Moves on boxes change the upper lefthand corner.
#   * Moves on anchored objects change the anchor point.
#   mhuelsman

# Version 5.1 8/26/2018
#   * Added a getCenter method to the Polygon class
#   aalbina
...
class GraphicsObject:
    ...
    def __init__(self, options):
        ...
        # Add in an achor point object for use with move_to
        self.anchor = None
    ...
    def move_to(self, x, y):
        """"move object directly to coordinate (x,y)"""
        self._move_to(x, y)
        canvas = self.canvas
        if canvas and not canvas.isClosed():
            ...
            if self.anchor is not None:
                self.canvas.move(self.id, x - self.anchor.x, y - self.anchor.y)
            if canvas.autoflush:
                _root.update()
    ...
    def move(self, dx, dy):
        ...
        if canvas and not canvas.isClosed():
            ...
            if trans:
                ...
                x = dx
                y = dy
            curr = self.canvas.coords(self.id)
    ...
    def _move_to(self, x, y):
        """updates the internal state of object to move it to x,y"""
        pass # must override in subclass
...
class Circle(Oval):
    ...
    def move_to(self, x, y):
        super().move_to(x-self.radius, y-self.radius)
...
class Polygon(GraphicsObject):
    ...
    def getCenter(self):
        ##By: Robert Fotino: https://fotino.me/calculating-centroids/
        ##Modified for python3, and inclusion in the graphics module by Adam R. Albina
        """Calculates the centroid of a non-intersecting polygon.
        Args: poly: a list of points, each of which is a list of the form [x, y].
        Returns: the centroid of the polygon in the form [x, y].
        Raises: ValueError: if poly has less than 3 points or the points are not formatted correctly.
        """
        # Get the points of the polygon from self and create a list of lists of x,y pairs. ARA
        points = self.getPoints()
        poly = []
        for pt in points:
            poly.append([pt.getX(), pt.getY()])
        ## End modifications ARA
        ...
        # Calculate the centroid from the weighted average of the polygon's constituent triangles
        area_total = 0
        centroid_total = [float(poly[0][0]), float(poly[0][1])]

        for i in range(0, len(poly) - 2):  ##Modified for Python 3 rangex not supported ARA
            ...
            # If the area is zero, the triangle's line segments are colinear so we should skip it
            if area == 0:
                continue
            ...
            centroid_total[0] = ((area_total * centroid_total[0]) + (area * centroid[0])) / (area_total + area)
            centroid_total[1] = ((area_total * centroid_total[1]) + (area * centroid[1])) / (area_total + area)
            area_total += area

        ##Return the Point Object that all the other methods need ARA
        return (Point(centroid_total[0], centroid_total[1]))
